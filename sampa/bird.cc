...
void bird_calculate(bird_data *b) {
  ...
  if ((b->zenith >= 0) && (b->zenith < 90) && (b->r > 0)) {
    ...
    t_rayliegh =
        exp(-0.0903 * pow(press, 0.84) * (1 + press - pow(press, 1.01)));
    t_ozone = 1 - 0.1611 * (oz)*pow(1 + 139.48 * oz, -0.3034) -
              0.002715 * (oz) / (1 + 0.044 * (oz) + 0.0003 * oz * oz);
    t_gases = exp(-0.0127 * pow(press, 0.26));
    t_water = 1 - 2.4959 * wat / (pow(1 + 79.034 * wat, 0.6828) + 6.385 * wat);
    t_aerosol =
        exp(-(pow(b->taua, 0.873)) * (1 + b->taua - pow(b->taua, 0.7088)) *
            pow(b->amass, 0.9108));

    b->direct_normal =
        0.9662 * etrn * t_aerosol * t_water * t_gases * t_ozone * t_rayliegh;

    taa = 1 - 0.1 * (1 - b->amass + pow(b->amass, 1.06)) * (1 - t_aerosol);
    rs = 0.0685 + (1 - b->ba) * (1 - t_aerosol / taa);
    ias = etrn * coszen * 0.79 * t_ozone * t_gases * t_water * taa *
          (0.5 * (1 - t_rayliegh) + b->ba * (1 - (t_aerosol / taa))) /
          (1 - b->amass + pow(b->amass, 1.02));

    bird_horiz(b->direct_normal, coszen, ias, b->albedo, rs, &b->global_horiz,
               &b->diffuse_horiz);

    if (b->dni_mod >= 0) {
      b->direct_normal_mod = b->direct_normal * b->dni_mod;
      bird_horiz(b->direct_normal_mod, coszen, ias, b->albedo, rs,
                 &b->global_horiz_mod, &b->diffuse_horiz_mod);
    }
